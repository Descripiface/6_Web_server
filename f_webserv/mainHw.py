...
import os  # сокеты, получение путей и размера файлов, время для полей и логов
import socket
import time

sock = socket.socket()

with open('settings.ini', 'r') as f:  # файл с настройками сервера
    settings = f.read()
...
while True:
    ...
    if askfile == '/':  # если просят корень
        with open(os.path.join(workdir, 'hello.html'), 'r', encoding='UTF-8') as f:
            ansfile = f.read()
        ...
        if os.path.exists(os.path.join(workdir, askfile)):  # если файл существует (путь)
            if os.path.isfile(os.path.join(workdir, askfile)):  # если это файл (иначе кидаем 403)
                if ext == 'html':  # далее сравнения по расширениям
                    ...
                    with open(os.path.join(workdir, askfile), 'r') as f:
                        ansfile = f.read()
                    header[2] = '200 OK'
                    header[4] = 'text/txt'
                    answer = ''.join(header) + ansfile
